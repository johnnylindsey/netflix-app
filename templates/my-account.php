        ...
        <div class="row">
            <div class="col-xs-8 mx-auto">
                <table class="table" style="color: white;">
                    ...
                    <tbody>
                        <?php
                        $cdat = $this->db->query("select commentText, showID from comment where username = ?", "s", $user["username"]);

                        $count = 1;
                        foreach ($cdat as $c) {
                            $title = $this->db->query("select movieName from movie where showID = ?", "s", $c["showID"]);
                            foreach ($title as $t) {
                                echo "<tr><th scope='row'>" . $count . "</th><td>" . $c["showID"] . "</th><td>" . $t["movieName"] . "</th><td>" . $c['commentText'] . "</td></tr>";
                                $count = $count + 1;
                            }
                        }
